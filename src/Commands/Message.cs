...
namespace Th3Essentials.Commands
{
  internal class Message : Command
  {
    ...
    private Dictionary<string, string> LastMsgFrom = new Dictionary<string, string>();
    ...
    internal override void Init(ICoreServerAPI api)
    {
      if (Th3Essentials.Config.MessageEnabled)
      {
        ...
        api.RegisterCommand("r", Lang.Get("th3essentials:cd-reply"), Lang.Get("th3essentials:cd-reply-param"),
        (IServerPlayer player, int groupId, CmdArgs args) =>
        {
          string msgRaw = args.PopAll();
