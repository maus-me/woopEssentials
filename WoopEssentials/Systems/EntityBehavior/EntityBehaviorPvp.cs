...
public class EntityBehaviorPvp : Vintagestory.API.Common.Entities.EntityBehavior
{
    private const string AttrKey = "woopessentials-pvp-enabled";
    private const string CooldownAttrKey = "woopessentials-pvp-cooldown-until"; // stores UTC ticks (long)
    ...
    public override void OnEntityReceiveDamage(DamageSource damageSource, ref float damage)
    {
        ...
        if (damage <= 0) return;
        if (damageSource.Source != EnumDamageSource.Player) return;
        ...
        if (attacker == null) return;
        {
            ...
            try
            {
                ...
                // Notify victim
                if ((entity as EntityPlayer)?.Player is IServerPlayer victimSp)
                {
    ...
    private bool IsPvP(out string? errorMessage)
    {
    ...
    public static bool CheckPvP(IPlayer player, out string? errorMessage)
    {
        ...
        if (pvpBehavior == null) return true;
        return pvpBehavior.IsPvP(out errorMessage);
    }
