...
using WoopEssentials.Discord.Commands;

namespace WoopEssentials.Discord;
...
public abstract class WoopSlashCommands
{
    public static void CreateGuildCommands(DiscordSocketClient client, ICoreServerAPI sapi)
    {
        ...
        var created = client.Rest.BulkOverwriteGuildCommands(commands, WoopDiscord.Instance.Config.GuildId).GetAwaiter().GetResult();
            
        sapi.Logger.Notification($"Discord Slashcommands created: {string.Join(", ", created.Select(c => c.Name))}");
    }

    internal static void HandleButtonExecuted(WoopDiscord discord, SocketMessageComponent component)
    {
        string response;
    ...
    internal static async void HandleSlashCommand(WoopDiscord discord, SocketSlashCommand commandInteraction)
    {
        ...
        if (Enum.TryParse(commandInteraction.Data.Name, true, out SlashCommands cmd))
        {
            switch (cmd)
            {
                ...
                {
                    response = WoopUtil.GetAdmins(discord.Sapi);
                    break;
                }
