...
{
    private const string REWARDS_SERVER_DATA_KEY = "wooprewards";
    private Harmony? _harmony;

    private readonly string _harmonyPatchkey = "WoopEssentials.Discord.Patch";
    ...
    private Task MessageReceivedAsync(SocketMessage messageParam)
    {
        ...
        if (message.Content.ToLower().StartsWith("!setupwoopessentials"))
        {
    ...
    private void PlayerNowPlayingAsync(IServerPlayer byPlayer)
    {
        ...
        foreach (KeyValuePair<string, string> role in Config.RewardIdToName)
        {
            ...
            if (socketRole != null)
            {
                // Sapi.Logger.VerboseDebug($"{WoopEssentials.ToHex(socketRole.Color)}, {socketRole.Name}, {socketRole.Id}");
